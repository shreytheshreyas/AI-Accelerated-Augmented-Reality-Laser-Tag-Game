...
class StateStaff(PlayerStateBase):
    ...
    def update(
        ...
        if self.shield_time > 0:
            self.shield_time = max(
                self.shield_max_time - (time.time() - self.shield_start_time), 0
            )
    ...
    def action_is_valid(self, action_self, opp_hit, opp_blasted):
        ret = True
        # check if the shield has to reduced
        if action_self == Actions.shield:
            if self.shield_time > 0:
            ...
            if self.bullets <= 0 or not opp_hit:
                # invalid
                ret = False

        elif action_self == Actions.grenade:
            if self.grenades <= 0 or not opp_blasted:
                # invalid
                ret = False

...
class TurnGenerator:
    ...
    def __init__(self):
        ...
        self.game_engine = MoveEngine(
            is_single_player=SINGLE_PLAYER_MODE, disable_gui=DISABLE_GUI
        )
    ...
    def get_display_position(self):
        ...
        """
        return str(self.p1_positions[self.cur_turn]), str(
            self.p2_positions[self.cur_turn]
        )
...
class Server(threading.Thread):
    ...
    def __init__(self, port_num, group_id):
        ...
        self.columns = [
            ...
            "gt_p2_action",
            "response_time",
            "is_p1_action_correct",
            "is_p2_action_correct",
        ]
        ...
        self.action_set_time = (
            0  # Time turn instructions/actions were set by eval_server
        )
        self.turn_wait_timeout = 60  # Turn response timeout amount
        self.turn_wait_timer = (
            None  # Timer object to keep track of turn response timeout
        )
    ...
    def iterate(self):
        ...
        if finished:
            ...
            (
                self.last_p1_action,
                self.last_p2_action,
            ) = self.turn_gen.get_correct_action()
    ...
    def write_turn_to_logger(
        self,
        predicted_p1_action: str,
        correct_p1_action: str,
        predicted_p2_action: str,
        correct_p2_action: str,
        game_state_received: dict,
        game_state_expected: dict,
    ):
        """
        ...
        if _flag_write_csv:
            ...
            with open(log_filepath, "a") as f:
                ...
                self.df = pd.DataFrame(data, index=[0])[self.columns].set_index(
                    "timestamp"
                )
            ...
            try:
                ...
                if True:
                    diff_p1 = self.game_state_diff(
                        _game_state_correct.player_1.get_dict(),
                        game_state_received["p1"],
                    )
                    diff_p2 = self.game_state_diff(
                        _game_state_correct.player_2.get_dict(),
                        game_state_received["p2"],
                    )
...
class Eval:
    ...
    def check_loop_status(self):
        ...
        self.display_window.after(
            10, self.check_loop_status
    ...
    def start(self):
        ...
        if not DISABLE_GUI:
            ...
            main_frame = tk.Frame(self.display_window)
            ...
            # Player 1 title
            p1_label = tk.Label(
                self.p1_frame, text="Player 1", font=("times", 100, "bold"), bg="red"
            )
            ...
            p1_action_label = tk.Label(
                p1_var_frame,
                textvariable=self.p1_action_text,
                font=("times", 100),
                bg="#ffcccb",
            )
            ...
            if not SINGLE_PLAYER_MODE:
                ...
                p2_action_label = tk.Label(
                    p2_var_frame,
                    textvariable=self.p2_action_text,
                    font=("times", 100),
                    bg="#add8e6",
                )
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < _num_para:
        ...
        DISABLE_GUI = True
        print(
            "This is the eval server meant for terminal mode, this is for students to tryout their eval client"
        )
